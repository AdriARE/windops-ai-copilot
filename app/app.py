...
with st.sidebar:
    ...
    if "live_mode_unlocked" not in st.session_state:
        st.session_state["live_mode_unlocked"] = False

    if st.session_state["live_mode_unlocked"]:
        st.success("🔓 Live mode unlocked")
        if st.button("Lock live mode"):
            st.session_state["live_mode_unlocked"] = False
            st.rerun()
    else:
        st.caption("Locked — agent runs in demo mode until unlocked.")
        pwd = st.text_input("Password", type="password", key="live_mode_pwd")
        if st.button("Unlock live mode"):
            import os
            if pwd and pwd == os.environ.get("LIVE_MODE_PASSWORD", ""):
                st.session_state["live_mode_unlocked"] = True
                st.rerun()
            else:
                st.error("Incorrect password.")

    allow_live = st.session_state["live_mode_unlocked"]
    ...
    if run_agent or st.session_state["agent_results"] is not None:
        ...
        if run_agent or st.session_state["agent_results"] is None:
            with st.spinner("Agent is analysing the fleet..."):
                import time
                t0 = time.time()
                plans, trace, mode = run_agent_auto(priority, df_risk, top_n=top_n, allow_live=allow_live)
        ...
        if mode == "live":
            ...
            st.info("🔵 DEMO MODE — live mode is locked. Unlock it in the sidebar to use the Anthropic API.")
        else:
